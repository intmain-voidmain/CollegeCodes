...
class QGen:
    ...
    def predict_shortq(self, payload):
        ...
        keywords = get_keywords(self.nlp,modified_text,inp['max_questions'],self.fdist,self.normalized_levenshtein,len(sentences) )
        ...
        for k in keyword_sentence_mapping.keys():
            ...
            # print(keyword_sentence_mapping[k])
...
class FillQGen:
    ...
    def predict_fillq(self, payload):
        inp = {
            "input_text": payload.get("input_text"),
            "max_questions": payload.get("max_questions", 4)
        }
        ...
        for keyword, sentences in keyword_sentence_mapping.items():
            for sentence in sentences:
                blanked_sentence = sentence.replace(keyword, '_____')
                questions.append(blanked_sentence)
        return questions